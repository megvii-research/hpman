...
import glob
import os
from typing import Optional, Set, Dict, Mapping, Union, List
...
TreeMapping = Mapping[str, Union[Primitive]]
...
class HyperParameterManager:
    ...
    def get_tree(self) -> TreeMapping:
        ...
        :raises: ValueError
        """
        def _create_tree(db: dict, keys: List[str], val) -> dict:
            if len(keys) == 0:
                return val
            key, *rest = keys
            if key in db:
        ...
        for i, key in enumerate(keys[:-1]):
            ...
            for test_key in keys[i+1:]:
                ...
                if t_tokens[:len(tokens)] == tokens:
                    ...
                    raise ValueError(
                        "Impossible tree: both `{}` and `{}` are set."
                        .format(key, test_key)
                    )
